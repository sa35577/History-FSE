...
yy = image.load("l2.png")
Z = image.load("l2.png")
screen.blit(image.load("realTwo.png"),(0,0))
scroll = image.load("scroll_left.png")
scroll = transform.scale(scroll,(50,62))
x = screen.copy()
scrolls = [[225,90],[847,492],[925,735]]
DONE = set()
game_over = False
for s in scrolls:
    s[1] += 10
    ...
    try:
        ...
        if m >= 0:
            if scrollRect.colliderect(player) and di[m] == False:
                subprocess.call("Two%i.py" % m,shell = True)
                #print(m)
                DONE.add(m)
                di[m] = True
        if len(DONE) == 3 and GUY[X] > 2080:
            game_over = True
            screen.fill(BLACK)
            floweyFont("You Finished the second level!",300)
            display.flip()
            time.wait(2000)
            quit() 
