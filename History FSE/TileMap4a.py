...
def mask(screen,GUY,w):
    ...
    try:
        ...
        if m >= 0:
            if scrollRect.colliderect(player) and not di[m]:
                subprocess.call("Four%i.py" % m,shell = True)
                #print(m)
                DONE.add(m)
                di[m] = True
        ...
        if len(DONE) == 3 and GUY[X] > 2080:
            game_over = True
            screen.fill(BLACK)
            floweyFont("You Finished the Game...... Congratulations!",300)
            display.flip()
            time.wait(2000)
            quit()
...
##    if game_over:
##        screen.fill(BLACK)
##        floweyFont("You Finished the Game..... Congratualations",300)
##        display.flip()
