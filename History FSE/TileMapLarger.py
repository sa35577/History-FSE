...
MAP = Rect(50,50,8532,2736) #MAP
draw.rect(screen,GREEN,MAP)
cx = int(80*7.2)
cy = int(70*7.2)
tiles = [Rect(cx-144,cy,144,216),Rect(cx,cy,144,216)]
...
while cx < 1180*7.2:
    ...
    if cdir == 0:
        ...
        for i in range(x):
            if cx < 1200*7.2:
                tiles.append(Rect(cx+144,cy,144,216))
                cx += 144
    elif cdir == 1:
        y = randint(2,3)
        for i in range(y):
            if cy < 390*7.2:
                tiles.append(Rect(cx,cy+30,144,216))
                cy += 30
    elif cdir == 2:
        y = randint(2,3)
        for i in range(y):
            if cy > 130*7.2:
                tiles.append(Rect(cx,cy-216,144,216))
                cy -= 216
...
for i in range(4):
    sprites[i] = transform.scale(sprites[i],(360,446))
GUY = [720,648]
POS = [720,936]
X = 0
Y = 1
